...
// ==========================================
// THAY LINK GOOGLE SCRIPT MỚI NHẤT CỦA BẠN VÀO ĐÂY
...
// Cập nhật background cho đúng luồng mới
const stageBackgrounds = {
    0: "linear-gradient(to bottom, #E0F7FA, #B2EBF2)", // Xanh nhạt
    1: "linear-gradient(to bottom, #a5d6a7, #e8f5e9)",
    2: "linear-gradient(to bottom, #e0f7fa, #e0f7fa)",
    4: "linear-gradient(to bottom, #fff9c4, #fff176)", // Capybara
    5: "linear-gradient(to bottom, #87CEEB 0%, #E0F7FA 100%)", // Mây
    'pain-map': "linear-gradient(to bottom, #ffebee, #ffcdd2)", // Nỗi đau
    3: "linear-gradient(to bottom, #e0f2f1, #b2dfdb)", // Body scan
    6: "linear-gradient(to top, #fce4ec, #f8bbd0)", // Hũ
...
function startGameDirectly() {
    ...
    setTimeout(() => {
        ...
        document.getElementById('stage-0').style.display = 'none'; document.getElementById('stage-0').classList.remove('active');
        const emotionStage = document.getElementById('stage-emotion-check');
        emotionStage.style.display = 'flex'; emotionStage.classList.add('active');
...
// ==========================================
const emotionLevels = {
    1: { text: "Tuyệt vọng", emoji: "😭", color: "#1a237e" }, 2: { text: "Rất tồi tệ", emoji: "😫", color: "#4a148c" }, 3: { text: "Tồi tệ", emoji: "😠", color: "#b71c1c" }, 4: { text: "Kém", emoji: "☹️", color: "#e53935" }, 5: { text: "Bình thường (Ổn)", emoji: "😐", color: "#f57f17" }, 6: { text: "Tương đối tốt", emoji: "🙂", color: "#fbc02d" }, 7: { text: "Tốt", emoji: "😊", color: "#fdd835" }, 8: { text: "Rất tốt", emoji: "😁", color: "#c0ca33" }, 9: { text: "Tuyệt vời", emoji: "😄", color: "#66bb6a" }, 10: { text: "Rất tuyệt vời", emoji: "🤩", color: "#00c853" }    
};

function updateEmotionDisplay() {
    const slider = document.getElementById('emotion-range'); if(!slider) return;
    const val = parseInt(slider.value); const data = emotionLevels[val];
    document.getElementById('current-emoji').innerText = data.emoji;
    document.getElementById('current-status').innerText = `${val} - ${data.text}`; document.getElementById('current-status').style.color = data.color;
...
function submitInitialEmotion() {
    ...
    if (slider) {
        const val = parseInt(slider.value); currentUser.initialMood = emotionLevels[val] ? emotionLevels[val].text : val;
        localStorage.setItem('user_' + currentUser.phone, JSON.stringify(currentUser));
    }
    switchStage(1);
}
...
function switchStage(stageNum) {
    ...
    if (newStage) {
        newStage.style.display = 'flex'; setTimeout(() => { newStage.classList.add('active'); }, 10);
...
class Particle {
    constructor(x, y, color) { this.x = x; this.y = y; this.color = color; this.radius = Math.random() * 3 + 1; this.velocity = { x: (Math.random() - 0.5) * 8, y: (Math.random() - 0.5) * 8 }; this.alpha = 1; this.friction = 0.95; }
...
function animateFireworks() {
    ...
    requestAnimationFrame(animateFireworks); ctx.fillStyle = 'rgba(255, 255, 255, 0.1)'; ctx.clearRect(0, 0, canvas.width, canvas.height);
...
// 6. STAGE 1: KHỈ
// ==========================================
const sceneS1 = document.getElementById('scene-s1'); const guideTextS1 = document.getElementById('guide-text-s1'); const countdownDisplay = document.getElementById('countdown-display'); const stopBtn = document.getElementById('stop-btn'); const successPanel = document.getElementById('success-panel');
let s1_monkeys = []; let s1_timer = null; let s1_count = 0; let s1_isSuccess = false;
function createMonkeys(amount) {
    for (let i = 0; i < amount; i++) {
        const monkey = document.createElement('div'); monkey.classList.add('monkey', 'running'); monkey.innerText = Math.random() > 0.5 ? '🐒' : '🙉';
        monkey.style.left = (Math.random() * 90) + '%'; monkey.style.top = (Math.random() * 80) + '%'; monkey.style.animationDuration = (Math.random() * 1.5 + 1) + 's';
        sceneS1.appendChild(monkey); s1_monkeys.push(monkey);
    }
}
function resetStage1() {
    if(successPanel) successPanel.style.display = 'none'; document.getElementById('top-message-area').innerHTML = ''; document.getElementById('greeting-text').style.opacity = '1'; document.getElementById('monkey-metaphor').style.opacity = '1';
    s1_isSuccess = false; clearInterval(s1_timer); countdownDisplay.innerHTML = ''; stopBtn.style.display = 'flex'; guideTextS1.innerText = 'Nhấn giữ chuông để ra lệnh\n"DỪNG LẠI"'; guideTextS1.style.opacity = '1';
...
function cancelProcess() {
    if (s1_isSuccess) return; clearInterval(s1_timer); countdownDisplay.innerHTML = ''; guideTextS1.innerText = 'Nhấn giữ chuông để ra lệnh\n"DỪNG LẠI"'; guideTextS1.style.opacity = 1; s1_count = 0;
...
function initDragon() { 
    ...
    currentUser.dragonBreaths = 0; // Luôn mặc định là 0
    if(fire) fire.classList.remove("active"); if(belly) belly.classList.remove("inhaling"); 
}
...
function startBreath(e) {
    ...
    // Đã xóa/comment dòng đếm nhịp thở
    // currentUser.dragonBreaths += 1; 
    
    s2_isHolding = true; s2_energy = 0; 
    instructionDragon.textContent = "Hít sâu..."; instructionDragon.style.color = "#4caf50"; dragonBtn.textContent = "Đang hít vào..."; 
...
function initPainMap() {
    selectedPainsThisSession = {}; const container = document.getElementById('pain-map-svg-container'); if (!container) return;
    container.innerHTML = `
    ...
    painAreasConfig.forEach(area => {
        const dot = document.createElement('div'); dot.style.position = 'absolute'; dot.style.top = area.top; dot.style.left = area.left; dot.style.width = '30px'; dot.style.height = '30px';
...
function submitPainMap() {
    ...
    for (let id in selectedPainsThisSession) { let name = selectedPainsThisSession[id]; if (!painHistory[id]) painHistory[id] = 0; painHistory[id] += 1; finalResultArray.push(`${name} (${painHistory[id]} lần)`); }
...
function initBodyScan() {
    s3_currentStep = 0; if(actionButtonsBody) actionButtonsBody.style.display = 'none'; if(!containerBody) return; containerBody.innerHTML = '';
...
function saveToJar() {
    ...
    currentUser.jarNote = msg; writeJar.style.opacity = '0'; setTimeout(() => { writeJar.style.display = 'none'; jarScreenFinal.style.display = 'flex'; triggerDroppingHeart(); }, 500);
}
...
function finishJourney() {
    ...
    // TẠO BẢN SAO DỮ LIỆU ĐỂ GỬI & FORMAT LẠI THỜI GIAN THEO GIỜ VN
    ...
    let hh = String(now.getHours()).padStart(2, '0'); let min = String(now.getMinutes()).padStart(2, '0'); let ss = String(now.getSeconds()).padStart(2, '0');
    ...
    .then(data => { 
        if(data.result === 'success') { alert("Tuyệt vời! Hành trình bình an của chị đã được lưu lên Google Sheet."); } 
        else { alert("Có lỗi khi lưu lên Sheet: " + data.error); }
        location.reload(); 
    })
    .catch(error => { console.error("Lỗi mạng: ", error); alert("Đã hoàn thành! (Dữ liệu đã lưu tạm trên thiết bị do lỗi mạng)."); location.reload(); });
}

// Hàm quay lại với thứ tự mới: 0 -> em check -> 1 -> 2 -> 4 -> 5 -> pain-map -> 3 -> 6 -> eval-1 -> eval-2 -> 7
function goBack() {
...
// ==========================================
function openAdminPanel() { const modal = document.getElementById('admin-modal'); if(modal) { modal.style.display = 'block'; fetchHistory(); } }
function closeAdmin() { const modal = document.getElementById('admin-modal'); if(modal) modal.style.display = 'none'; const phoneInput = document.getElementById('input-phone'); if(phoneInput) phoneInput.value = ""; }
function fetchHistory() {
    ...
    history.slice().reverse().forEach((row, index) => {
        ...
        tr.innerHTML = `
            <td style="padding: 10px; border: 1px solid #ddd;">${row.created_at}</td>
...
function showMyHistory() {
    const modal = document.getElementById('my-history-modal'); if(modal) modal.style.display = 'block';
    let history = JSON.parse(localStorage.getItem('myJourneys')) || []; let myData = history.filter(item => item.phone === currentUser.phone);
    const content = document.getElementById('my-history-content'); if(!content) return;
    ...
    let html = ''; myData.slice().reverse().forEach(row => {
        html += `<div style="background: #fdfaf6; padding: 15px; border-radius: 10px; margin-bottom: 15px; border-left: 5px solid #00897b; box-shadow: 0 2px 5px rgba(0,0,0,0.05);"><div style="font-size: 14px; color: #888; margin-bottom: 10px;">📅 ${row.created_at}</div><div style="display: flex; justify-content: space-between; background: #fff; padding: 10px; border-radius: 8px;"><div>🌱 <b>Đầu:</b> ${row.initialMood || '...'}</div><div>✨ <b>Cuối:</b> <span style="color: #e91e63; font-weight:bold;">${row.finalMood || '...'}</span></div></div><p>🦁 <b>Bé Capybara:</b> ${row.capybaraMood || '...'}</p><p>☁️ <b>Suy nghĩ:</b> <i>"${row.cloudThought || '...'}"</i></p><div style="background: #e0f2f1; padding: 10px; border-radius: 8px; margin-top: 10px; color: #004d40;">💌 <b>Lời nhắn:</b><br>"${row.jarNote || '...'}"</div></div>`;
    }); content.innerHTML = html;
}
function closeMyHistory() { const modal = document.getElementById('my-history-modal'); if(modal) modal.style.display = 'none'; }
