...
function startGameDirectly() {
    ...
    emotionStage.style.display = 'block';
    setTimeout(() => emotionStage.classList.add('active'), 10);
}
...
function gameLoopS2() {
    ...
    if (s2_isHolding) { 
        s2_speed = s2_speed * 0.9; if (s2_speed < 0.1) s2_speed = 0; if (s2_energy < 100) s2_energy += 0.5; 
    } else { 
        if (isBlowing) { s2_speed *= 0.995; if (s2_speed < 8) s2_speed = 8; } else { if (s2_speed > 0) s2_speed *= 0.96; if (s2_speed < 0.1) s2_speed = 0; }
    }
    s2_rotation += s2_speed; if(pinwheel) pinwheel.style.transform = `rotate(${s2_rotation}deg)`;
...
function releaseBreath(e) {
    ...
    fireTimeout = setTimeout(() => {
        if (!s2_isHolding) { fire.classList.remove("active"); mouth.className = "mouth smile"; instructionDragon.textContent = "Hít vào..."; instructionDragon.style.color = "#006064"; }
    }, 2000); 
}
...
window.removeEventListener('mouseup', releaseBreath); window.removeEventListener('touchend', releaseBreath);
...
const containerBody = document.getElementById('meditation-container'); const guideTextBody = document.getElementById('guide-text-body'); const actionButtonsBody = document.getElementById('action-buttons-body');

function initBodyScan() {
    s3_currentStep = 0;
    if(actionButtonsBody) actionButtonsBody.style.display = 'none'; containerBody.innerHTML = '';
    
    const svgHTML = `<svg id="pregnant-standing-svg" viewBox="0 0 320 480" xmlns="http://www.w3.org/2000/svg" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; z-index: 1; transition: all 1.2s ease-in-out;">
        <path d="M 130 60 Q 160 40 190 60 L 195 90 Q 160 100 125 90 Z" class="hair-tone" fill="#5D4037" />
        <rect x="135" y="430" width="20" height="50" class="skin-tone" fill="#FFCCBC" />
        <rect x="165" y="430" width="20" height="50" class="skin-tone" fill="#FFCCBC" />
        <rect x="150" y="105" width="20" height="25" class="skin-tone" fill="#FFCCBC" />
        ...
        <path class="skin-tone" d="M 120 135 Q 100 195 140 245 L 155 255 L 120 135 Z" fill="#FFCCBC" />
        <path class="skin-tone" d="M 200 135 Q 220 195 180 245 L 165 255 L 200 135 Z" fill="#FFCCBC" />
        <circle cx="160" cy="70" r="38" class="skin-tone" fill="#FFCCBC" />
        ...
        <path d="M 124 60 Q 160 75 196 60 Q 196 45 160 40 Q 124 45 124 60 Z" class="hair-tone" fill="#5D4037" />
    </svg>`;
...
function handleDotClick(idx) {
    ...
    if(sadFace) sadFace.style.display = 'none'; if(happyFace) happyFace.style.display = 'block';
    clearTimeout(faceTimeout); 
    faceTimeout = setTimeout(() => { if(sadFace) sadFace.style.display = 'block'; if(happyFace) happyFace.style.display = 'none'; }, 3000); 
...
function selectEmotion(name) {
    ...
    s1.style.display = 'none'; s2.style.display = 'block'; setTimeout(() => s2.style.opacity = '1', 50); 
}
...
function createCloud(e) {
    ...
    hintTextCloud.innerText = `Thở ra và quan sát đám mây trôi cùng cảm xúc ${txt}...`; hintTextCloud.classList.add('show'); s5_isHidden = true;
    ...
    setTimeout(() => { 
        wrap.remove(); 
        if(s5_isHidden) { inputContainerCloud.classList.remove('hidden'); hintTextCloud.classList.remove('show'); setTimeout(() => hintTextCloud.innerText = "Chạm vào bầu trời để viết tiếp...", 500); s5_isHidden=false; } 
    }, 20000);
}
...
function goToWrite() {
    ...
    setTimeout(() => { introJar.style.display = 'none'; writeJar.style.display = 'block'; setTimeout(() => writeJar.style.opacity='1', 50); }, 500);
}
function saveToJar() {
...
function triggerDroppingHeart() {
    const flyingHeart = document.createElement('div'); flyingHeart.classList.add('falling-heart', 'animate-drop');
    jarScreenFinal.appendChild(flyingHeart);
    setTimeout(() => {
        flyingHeart.remove();
        const landedHeart = document.createElement('div'); landedHeart.className = 'heart-in-jar'; jar.appendChild(landedHeart);
        finalMsg.style.opacity = "1"; contBtnJar.style.display = "block"; setTimeout(() => contBtnJar.style.opacity = "1", 100);
...
function finishJourney() {
    ...
    history.push(currentUser); localStorage.setItem('myJourneys', JSON.stringify(history));

    console.log("Dữ liệu gửi lên server:", currentUser);
...
function goBack() {
    ...
    if (typeof currentStage === 'number' && currentStage > 0) {
...
// Khởi chạy: Hiện modal đăng nhập đầu tiên
